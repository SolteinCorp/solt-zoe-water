...
class PaymentProvider(models.Model):
    ...
    def _get_compatible_providers(self, *args, sale_order_id=None, website_id=None, **kwargs):
        ...
        """
        compatible_providers = super()._get_compatible_providers(*args, sale_order_id=sale_order_id, website_id=website_id, **kwargs)
        if sale_order_id:
            sale_order = self.env["sale.order"].browse(sale_order_id).exists()
            if sale_order.is_subscription:
                return compatible_providers.filtered(lambda provider: not provider.capture_manually)

...
class SaleOrderLine(models.Model):
    ...
    def _set_price_from_plan(self):
        ...
        pricing = self.env["solt.recurring.pricing"]._get_first_suitable_recurring_pricing(self.product_id, self.plan_id, pricelist)
        if pricing:
            ...
            self.price_unit = pricing.currency_id._convert(pricing.price, self.currency_id, self.company_id, fields.Date.today())
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if self.recurring_ok and (self.subscription_id or self.subscription_line_ids):
            invoice_line_values.update(
                {
                    "subscription_id": (self.subscription_id or self.subscription_line_ids.subscription_id).id,
                }
            )
    ...
    def _onchange_product_subscription(self):
        ...
        if not self.product_id or not self.product_id.recurring_ok:
            ...
            self.start_recurring_date = False

...
class PaymentToken(models.Model):
    ...
    def _handle_archiving(self):
        ...
        linked_subscriptions = self.env['solt.subscription'].search([('payment_token_id', 'in', self.ids)])
    ...
    def get_linked_records_info(self):
        ...
        subscriptions = self.env['solt.subscription'].search([('payment_token_id', '=', self.id)])
        for sub in subscriptions:
            res.append({
                ...
                'active_subscription': sub.state == 'active',
            })
        return res
